...
|---|---|---|---|---|---|---|
| `FMB hero vid.mp4` | MP4 H.264 · **3832×2160 · 30 fps · 6.73 s** · 21.2 MB · AAC stereo audio | Single shot: parking lot → arched entrance → open door → dining room (booths, pendants) | **Locked** | A | Hero video | Not 4K UHD (3832 wide, DAR 479:270) and not 20 s. Last frame ≠ first frame, so a raw loop would cut. Derivatives (see "Hero loop derivative" below): `hero-loop-1080.mp4` 5.6 MB (≥900 px), `hero-loop-720.mp4` 2.3 MB (phones), both **muted**, each ending in a 0.7 s dissolve to the first frame so `loop` is seamless; `hero-poster.jpg` (frame 0, 1920 w) + `hero-poster-960.jpg`. |
...
### Hero loop derivative (added Sept 17, 2026)
`public/media/hero-loop-1080.mp4` (5.6 MB) and `hero-loop-720.mp4` (2.3 MB): the full approved clip, muted, plus a 0.7 s dissolve from its last frames to its own first frame (ffmpeg `xfade`), total 6.80 s. Measured mean pixel difference between the derivative's last and first frame: 3.5 / 255 (identical to the eye) versus 69 / 255 for the raw clip, so `loop` restarts without a cut. No frames were regenerated. The earlier play-once files were removed.
