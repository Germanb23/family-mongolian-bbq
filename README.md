...
> **Status (Sept 17, 2026): local preview only. Not deployed.** Deployment, domain and hosting require explicit owner authorization — see `docs/business-intake-and-facts.md` §5 for the open items.
...
Same pattern as the Tacos El Vampiro site: build with `BASE_PATH=/<repo>` for a GitHub Pages project site (or unset for a root domain) and `SITE_URL=https://<host>` so canonical/OG URLs are emitted, then publish `dist/`. No repository, domain or hosting has been set up for this project yet.

```bash
MSYS_NO_PATHCONV=1 BASE_PATH=/family-mongolian-bbq SITE_URL=https://<user>.github.io npm run build
```
